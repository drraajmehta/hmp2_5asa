...
#uniref density #
#####################
unirefs_plotting <- rnauni_rotated_metab2 %>% select(matches("UniRef90_R6TIX3|UniRef90_C7H1G6|binary154|UniRef90_R6CZ24|UniRef90_R5CY66|UniRef90_T5S060"))
...
uni_density_outfile <- do.call("rbind",list(u1,u2,u3,u4,u5))
write.table(uni_density_outfile,"uni_dens_outfile.txt",sep="\t",quote=FALSE,row.names=FALSE)
...
uni_bar_outfile <- do.call("rbind",list(u_a,u_b,u_c,u_d,u_e))
write.table(uni_bar_outfile,"uni_bar_outfile.txt",sep="\t",quote=FALSE,row.names=FALSE)
...
#density
uni_dens_outfile <- read.table("/home/rsm34/5asabb3/submission/keyoutput/uni_dens_outfile.txt",header=T,sep="\t")
uni_dens_outfile$binary154 <- as.factor(uni_dens_outfile$binary154)
uni_dens_outfile$feature_name = factor(uni_dens_outfile$feature_name, levels = c("UniRef90_C7H1G6", "UniRef90_R6TIX3", "UniRef90_R6CZ24", "UniRef90_R5CY66","UniRef90_T5S060"))
...
ggsave(filename='/home/rsm34/5asabb3/submission/figures/fig3a_dens.png', plot=unirefdens, width = 3.4, height = 10.1, dpi = 600) 


#Bars
uni_bar_outfile <- read.table("/home/rsm34/5asabb3/submission/keyoutput/uni_bar_outfile.txt",header=T,sep="\t")
uni_bar_outfile$binary154 <- as.factor(uni_bar_outfile$binary154)

uni_bar_outfile$feature_name = factor(uni_bar_outfile$feature_name, levels = c("UniRef90_C7H1G6", "UniRef90_R6TIX3", "UniRef90_R6CZ24", "UniRef90_R5CY66","UniRef90_T5S060"))
...
ggsave(filename='/home/rsm34/5asabb3/submission/figures/fig3a_bar.png', plot=unirefbar, width = 1.5, height = 10, dpi = 600) 
...
gather196sig2  <- gather196sig %>% filter(gene == "UniRef90_R6TIX3"|gene=="UniRef90_C7H1G6"|gene=="UniRef90_U2QX51"|gene=="UniRef90_T5S060")
...
unirefcorr <-ggplot(gather196sig2 , aes(x = arcsqt_abundance, y = Nacetyl5ASA)) +
    geom_point(col = "#3C5488FF",alpha=0.8) + theme_bw()+ geom_smooth(method = "lm",
        ...
        #se = FALSE,
        size = 1) + facet_wrap(~ gene,ncol=2,scales = "free_x")+
	theme_cowplot(14) +
	theme(strip.background = element_blank(),axis.text.x = element_text(angle=45,vjust = 1, hjust = 1))+xlab("Rel. abund. (asin sqrt)")+
	scale_x_continuous(labels = function(x) format(x, scientific = TRUE)) #pdf 4x4 

ggsave(filename='/home/rsm34/5asabb3/submission/figures/fig3c.png', plot=unirefcorr , width = 4, height = 4, dpi = 600) 
