...
table1data2 <- table1data %>% 
	group_by(Participant.ID.x) %>% 
		...
		dx=getmode(diagnosis), chemo=max(Chemotherapy),hospital=max(as.numeric(hosp_2wks)),surgery=getmode(bowelsurg),calpro=max(fecalcal,na.rm=TRUE),sex=getmode(sex),race=getmode(race))
...
myvars <- c("age_consent", "age_dx","dx","abx", "hospital", "immuno","ster","bio","dysbiosis","sex","race","peds","surgery")
catvars<-c("abx", "hospital", "immuno","ster","bio","dysbiosis","sex","race","peds","dx","surgery")
...
write.csv(tab1mat,file="table1.csv") #to be formatted in word 
...
pq <- ggplot(concord, aes(x=IBDusers2, y=X154.0502_3.83)) + 
...
pq2 <- pq + scale_y_log10() + theme_bw() +labs(y= "Fecal 5-ASA level", x = "")+
	scale_x_discrete(labels=c("nonIBD" = "Non IBD", "IBDNon-User" = "IBD Non-user",
                              "User" = "5-ASA User")) + theme(text = element_text(size = 20))    

ggsave(filename='/home/rsm34/5asabb3/submission/figures/suppfig1.png', plot=pq2, width = 6, height = 6, dpi = 600) 
