...
group_names <- c(
  "Non-IBD (n=384)", "5-ASA non-users (n=432 samples)", "5-ASA users (n=609 samples)")
gene_pct <- data.frame(
...
#users 
#> prop.table(table(df_steroids$UniRef90_C7H1G6)) #.2187.
#        0         1
#0.5221675 0.4778325
#> prop.table(table(df_steroids$UniRef90_R6CZ24)) #0.238
#        0         1
#0.8259442 0.1740558
#> prop.table(table(df_steroids$UniRef90_R5CY66)) #0.012
#        0         1
#0.7931034 0.2068966
#> prop.table(table(df_steroids$UniRef90_T5S060))
#        0         1
...
genepct_gather <- gene_pct %>% gather(gene,value,2:5) %>% filter(Group=="5-ASA non-users (n=432 samples)"  | Group=="5-ASA users (n=609 samples)")
...
#### stats for bar graph ###
df_steroids_use <- df_steroids %>% select(SampleID, steroids, Participant.ID.x, UniRef90_C7H1G6, UniRef90_R5CY66, UniRef90_R6CZ24, UniRef90_T5S060)
df_steroids_use$binary154 <- "yes"

#import non-user proportion data
df_steroids_nonuse <-read.table("/home/rsm34/5asabb3/submission/keyoutput/prop_nonuse.txt",header=T,sep="\t")
df_steroids_nonuse$binary154 <- "no"


#concatenate (make sure 5-ASA use is labelled) 
combined_prop <- rbind(df_steroids_use,df_steroids_nonuse)
combined_prop$binary154 <- as.factor(combined_prop$binary154)

#test
proptest1 <- glmer(UniRef90_C7H1G6~ binary154 + (1|Participant.ID.x), data=combined_prop, family=binomial) #gives binary 
summary(proptest1)
proptest2 <- glmer(UniRef90_R5CY66~ binary154 + (1|Participant.ID.x), data=combined_prop, family=binomial) #gives binary 
summary(proptest2)
proptest3 <- glmer(UniRef90_R6CZ24~ binary154 + (1|Participant.ID.x), data=combined_prop, family=binomial) #gives binary 
summary(proptest3)
proptest4 <- glmer(UniRef90_T5S060~ binary154 + (1|Participant.ID.x), data=combined_prop, family=binomial) #gives binary 
summary(proptest4)

## way to see if people changed their steroid use over time (we already know that 5-ASA use changed over time)
combined_prop$steroids <- as.numeric(combined_prop$steroids)
aaa<-combined_prop %>% group_by(Participant.ID.x) %>% summarise(mymean=mean(steroids))
aaa %>% filter(mymean<2) %>% filter(mymean >1) #there are 25 of 68 participants who variably used steroids throughout the year long cohort 
